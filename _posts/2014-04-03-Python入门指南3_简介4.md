...
的，而循环只要条件执行（这里：b < 10），仍是如此。在Python和C一样，任何非零整数值为true; 零是假的。这种状况也可以是字符串或链表，事实上任何序列; 具有非零长度事情是真实的，空序列为假。在这个例子中使用的测试是一个简单的比较。标准的比较运算符写法相同，在C：<（小于），>（大于），== （等于），<= （小于或等于），> =（大于或等于）和！=（不等于）。
